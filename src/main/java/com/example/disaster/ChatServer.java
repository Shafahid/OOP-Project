        ...
        public void run() {
            try {BufferedReader reader = new BufferedReader(new InputStreamReader(socket.getInputStream()));
                ...
                if (clientName.equalsIgnoreCase("Admin")) {
                    adminUser = new ClientUser(clientName, socket, writer);
                    System.out.println("Admin is connected.");
                } else {
                    synchronized (clients) {
                        clients.add(new ClientUser(clientName, socket, writer));
                    }
                ...
                while ((message = reader.readLine()) != null) {
                    if (clientName.equalsIgnoreCase("Admin")) {
                        ...
                        if (parts.length == 2) {
                            ...
                            sendMessageToClient(targetClient, "Admin: " + adminMessage);
                        }
                    } else {
